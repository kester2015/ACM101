...
nListLength = length(nList);
eigenValueCollection = zeros(nListLength,2);

for ii = 1:nListLength
    [eigenValueCollection(ii,1), eigenValueCollection(ii,2)] = eigenA(nList(ii));
end

figure;
plot(nList, eigenValueCollection(:,1))
hold on
plot(nList, eigenValueCollection(:,2))
title("Eigen value against n");
xlabel("n");
ylabel("Eigen value")
legend(["min","max"])
